...
| 层 | 文件 | 依赖 | 覆盖 |
|---|---|---|---|
| 烟测 | `tests/smoke.mjs` (`npm test`) | 无需本地 RVC；会用真实 Edge TTS 联网合成 | 路由注册、Edge 合成、mock RVC 链路、分块渐进播放、文件代理、compact-index 代理、诊断、音色包注册表（sha256/代理/进度/多索引/卸载）、make-pack 工具 |
