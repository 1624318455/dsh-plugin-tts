    ...
    async handler(req, res) {
      ...
      try {
        ...
        writeErr(res, 502,
          ...
          `处理：打开 PowerShell，运行启动命令（详见《RVC 指南》→「启动本地 RVC 服务」）：\n` +
          `  <你的RVC目录>\\runtime\\python.exe <你的RVC目录>\\rvc-server.py --port 4892\n` +
          `并确认设置里的「服务地址」与端口一致。原始错误：${String((e && e.message) || e)}`,
          { code: "host.filesNeedsServer", params: { baseUrl, error: String((e && e.message) || e) } }
        );
      }
    }
  });
  ...
  const compactDisposer = webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        writeErr(res, 502,
          /无法连接本地 RVC 推理服务/.test(msg)
            ? `无法连接本地 RVC 服务——生成紧凑索引需要它先运行。` +
              `\n处理：打开 PowerShell 运行启动命令（详见《RVC 指南》→「启动本地 RVC 服务」）：` +
              `\n  <你的RVC目录>\\runtime\\python.exe <你的RVC目录>\\rvc-server.py --port 4892` +
              `\n原始错误：${msg}`
            : `紧凑索引生成失败：${msg}`,
          { code: /无法连接本地 RVC 推理服务/.test(msg) ? "host.compactNeedsServer" : "host.compactFail", params: { error: msg } }
        );
      }
    }
  });
