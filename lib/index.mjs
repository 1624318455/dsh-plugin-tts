...
// Return a short, platform-aware "how to start the local RVC service" hint.
// The host knows the OS; the client also translates the surrounding sentence
// via i18n, so the hint is deliberately command-oriented and kept language-light.
function rvcStartupHint() {
  const isWin = process.platform === 'win32';
  const isMac = process.platform === 'darwin';
  const dir = '<你的RVC目录>';
  const python = isWin ? `${dir}\\runtime\\python.exe` : `${dir}/runtime/bin/python`;
  const server = isWin ? `${dir}\\rvc-server.py` : `${dir}/rvc-server.py`;
  const base = [
    isWin
      ? 'Windows（PowerShell）：'
      : (isMac ? 'macOS 终端：' : 'Linux/macOS 终端：'),
    `  ${python} ${server} --port 4892`,
    '详见《RVC 指南》→「启动本地 RVC 服务」。'
  ];
  if (isMac) {
    base.unshift(
      'macOS 快速开始（如果已有 ~/rvc-work 手工环境）：',
      '  ~/rvc-work/venv/bin/python ~/rvc-work/rvc-server.py --port 4892',
      '或使用便携运行时（推荐给没有 RVC WebUI 的用户）：',
