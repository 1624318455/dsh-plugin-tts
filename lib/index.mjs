...
async function installPack(registry, packId) {
  ...
  let indexPath = '';
  if (current.version === pack.version && current.modelSha256 === model.sha256 && existsSync(modelPath)) {
    indexPath = pack.index && pack.index.url && existsSync(path.join(dir, 'index.index'))
      ? path.join(dir, 'index.index')
      : (current.indexPath || '');
    return { ok: true, skipped: true, name: pack.name, modelPath, indexPath };
  }
  mkdirSync(dir, { recursive: true });
  try {
    const modelBytes = await downloadVerified(model.url, model.sha256, model.size, '模型');
    writeFileSync(modelPath, modelBytes);
    if (pack.index && pack.index.url) {
      const indexBytes = await downloadVerified(pack.index.url, pack.index.sha256, pack.index.size, '索引');
      indexPath = path.join(dir, 'index.index');
      writeFileSync(indexPath, indexBytes);
    }
  } catch (e) {
    // remove partial install so a retry starts clean
    try {
      for (const f of [modelPath, path.join(dir, 'index.index')]) {
        rmSync(f, { force: true });
      }
  ...
  return { ok: true, skipped: false, name: pack.name, modelPath, indexPath, size: (model.size || 0) + (pack.index ? pack.index.size || 0 : 0) };
}
  ...
  const packInstallDisposer = webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        const result = await installPack(registry, packId);
