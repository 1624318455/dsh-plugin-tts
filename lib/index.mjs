...
const packProgress = new Map(); // key -> { phase, done, total, startedAt }

function setPackProgress(key, patch) {
  if (!key) return;
  const p = packProgress.get(key) || { phase: '等待', done: 0, total: 0, startedAt: Date.now() };
  Object.assign(p, patch);
  packProgress.set(key, p);
}

function clearPackProgress(key) {
  if (key) packProgress.delete(key);
}

/** Stream-download with incremental sha256 + progress; returns the full bytes. */
async function downloadStream(url, expectedSha256, expectedSize, label, onProgress) {
  const r = await fetch(url, { signal: AbortSignal.timeout(600000) });
  if (!r.ok) throw new Error(`${label} 下载失败（HTTP ${r.status}）`);
  const total = Number(r.headers.get('content-length')) || 0;
  if (!r.body) throw new Error(`${label} 下载流不可用`);
...
async function installPack(registry, packId, indexId, progressKey) {
  ...
  const packInstallDisposer = webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        const result = await installPack(registry, packId, indexId, progressKey);
    ...
    async handler(req, res) {
      try {
        ...
        if (p) {
          ...
          writeJson(res, 200, { phase: p.phase, done: p.done, total: p.total, speed: Math.round(p.done / elapsed) });
          if (Date.now() - p.startedAt > 600000) packProgress.delete(key);
          return;
        }
        writeJson(res, 200, { done: true });
      } catch (e) {
        writeJson(res, 500, { error: String((e && e.message) || e) });
      }
    }
