  ...
  const speakDisposer = webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        // synthesize") for RVC / Index-TTS2 / plain Edge TTS, reusing one pipeline:
        // RVC uses the calibration-probed chunk size; Index-TTS2 and Edge use a
        // fixed size (remote synthesis is the bottleneck, no local speed to probe).
