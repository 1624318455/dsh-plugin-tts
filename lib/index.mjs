...
// immediately, the rest warm in the background into job.ready. Local
// synthesis (rvc/index/cosy, ratio 0.5-1.1) keeps full prewarm so playback
// never starves.
...
export function apply(ctx) {
  ...
  // client toasts which segment was skipped. nextIdx only advances on success
  // or skip — order is never broken.
  ...
  function convertChunk(job) {
    const run = async () => {
      ...
      if (lastErr) {
        // Skip, don't kill the job: advance past the bad chunk so playback
        // continues; the client surfaces { skipped } as a toast.
        const skippedIdx = job.nextIdx;
        job.nextIdx++;
        const more = job.nextIdx < job.parts.length;
        if (!more) job.finishedAt = job.finishedAt || Date.now();
        const skipErr = String((lastErr && lastErr.message) || lastErr);
        recordHostError({
          provider: job.provider || '?',
          voice: job.voice,
          textLen: String(job.parts[skippedIdx] || '').length,
          stage: 'chunk ' + (skippedIdx + 1) + '/' + job.parts.length,
          error: skipErr.slice(0, 300)
        });
        return { skipped: skippedIdx + 1, error: skipErr, more };
      }
  ...
  function nextJobChunk(jobId) {
    ...
    // fastFirst: drain the background-prewarmed ready buffer first (same
    // order as on-demand synthesis; ready[i] always belongs to parts[i]).
    if (job.ready && job.ready.length) {
      const url = job.ready.shift();
      const more = job.nextIdx < job.parts.length || job.ready.length > 0;
      if (!more) job.finishedAt = job.finishedAt || Date.now();
      return Promise.resolve({ url, more });
    }
    ...
    async handler(req, res) {
      ...
      try {
        ...
        if (chunkable) {
          ...
          // fastFirst (edge/cloud only): TTFB = 1 block. Synthesize chunk 1,
// return immediately, warm the rest in the background into job.ready.
// Local synthesis (rvc/index/cosy, ratio 0.5-1.1) keeps full prewarm
// (no fastFirst) so playback never starves.
          ...
          if (parts.length > 1) {
            ...
            if (cal.fastFirst) {
              ...
              // half-baked job); the rest warm in the background through the
              // same serial tail chain, so order matches on-demand synthesis.
              // A background failure is swallowed here (nextIdx not advanced)
              // and the chunk is retried on demand at /rvc-next time.
