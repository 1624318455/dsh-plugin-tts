  ...
  const speakDisposer = webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        // synthesize") for RVC / Index-TTS2 / Cloud / plain Edge TTS, reusing
        // one pipeline: RVC uses the calibration-probed chunk size; Index-TTS2
        // and Edge use a fixed size (remote synthesis is the bottleneck, no
        // local speed to probe). Cloud uses splitCloudPackets (sentence split
        // + atomic protection from splitText, then byte packing under the
        // 5000-byte sync hard limit); short Cloud text stays in the single-URL
        // branch below as long as it fits in one packet.
  ...
  const diagnoseDisposer = webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        // 4) Google Cloud TTS: key presence + optional live probe.
