  ...
  const filesDisposer = webServer.register({
    ...
    async handler(req, res) {
      ...
      try {
        ...
        if (!r.ok) {
          writeJson(res, 502, { error: `RVC /files 接口异常（HTTP ${r.status}）：${(data && data.message) || r.statusText}` });
          return;
        }
        writeJson(res, 200, data || { ok: false, files: [] });
      } catch (e) {
        writeJson(res, 502, {
          error: `无法连接本地 RVC 服务（${baseUrl}）——浏览本地文件需要它先运行。\n` +
  ...
  const compactDisposer = webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        writeJson(res, 502, {
          error: /无法连接本地 RVC 推理服务/.test(msg)
