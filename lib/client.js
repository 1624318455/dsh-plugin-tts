...
window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const apply = ctx => {
      ...
      // with playback), so long RVC reads stream seamlessly.
      async function playChunks(jobId, initialUrls, token, onError) {
        const el = shared.audioEl;
      ...
      async function speakText(rawText, source, onError) {
        ...
        try {
          ...
          if (Array.isArray(result.chunks) && result.chunks.length) {
            playChunks(result.jobId, result.chunks, token, onError);
            return { ok: true, chunked: true, total: result.total || result.chunks.length };
          }
