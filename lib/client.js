...
"tab.voice": "Voice",
      ...
      async function rpcSpeak(text, voice, provider) {
        ...
        const timer = edgeOnly
          ...
          : null;
        const t0 = Date.now();
        const logSpeak = (ok, error) => logDiag({
          kind: "speak",
        ...
        try {
          ...
          logSpeak(!(result && result.error), result && result.error);
          return result;
        } catch (e) {
      ...
      function VoiceSettingsPanel() {
        ...
        const [diag, setDiag] = react.useState(null); // { running, checks, error, recent, worker, plugin }
        const runDiagnose = async () => {
          setDiag({ running: true, checks: null, error: null, recent: [], worker: null, plugin: null });
          try {
            ...
            setDiag({ running: false, checks: d.checks || [], error: null, recent: d.recent || [], worker: d.worker || null, plugin: d.plugin || null });
            return d;
          } catch (e) {
            setDiag({ running: false, checks: null, error: String((e && e.message) || e), recent: [], worker: null, plugin: null });
            return null;
          }
        };
        ...
        const buildDiagExport = payload => {
          ...
          const byKind = {};
          const countKind = k => { byKind[k] = (byKind[k] || 0) + 1; };
          ...
          const speakAttempts = clientRecent.filter(e => e.event === "speak").length;
          return {
            tool: "dsh-tts-diag",
            ...
            summary: {
              ...
              speakAttempts,
              speakFails: clientRecent.filter(e => e.event === "speak" && e.kind !== "speak-ok").length,
              byKind,
            },
            hostRecent,
            clientRecent,
        ...
        const exportDiagLog = async () => {
          try {
            const payload = diag && diag.checks
              ? { checks: diag.checks, recent: diag.recent || [], worker: diag.worker || null, plugin: diag.plugin || null }
              : await runDiagnose();
