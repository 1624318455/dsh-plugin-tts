...
"tab.voice": "Voice",
      ...
      const NODE_LIST_KEYS = ["nodes", "messages", "items", "events", "entries", "turns"];
      ...
      function pickNodes(snap) {
        ...
        for (const k of NODE_LIST_KEYS) {
          ...
          // object-map store: values ARE the nodes (id match still applies,
          // so location-like maps are harmless here)
          if (k === "nodes" || k === "messages" || k === "eventNodes") {
            const mv = mapValues(v);
            if (mv) return mv;
          }
        }
        if (snap.eventNodes && typeof snap.eventNodes === "object") {
          const mv = mapValues(snap.eventNodes);
          if (mv) return mv;
        }
      ...
      const AUTO_HOOKS = ["useConversation", "useChat", "useTrajectory",
