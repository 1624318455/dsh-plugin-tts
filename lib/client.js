      ...
      const I18N = {
        ...
        dict: {
          zh: {
            ...
            "diag.fail": "诊断失败：",
            "diag.copyLog": "复制诊断日志",
            "diag.copied": "诊断日志已复制，粘贴发给开发者即可",
            "diag.copyFail": "复制失败，请手动截图诊断区：",
            "packs.title": "音色包",
...
"tab.voice": "语音",
          ...
          en: {
            ...
            "diag.copyLog": "Copy diagnostic log",
            "diag.copied": "Diagnostic log copied — paste it to the developer",
            "diag.copyFail": "Copy failed, please screenshot the diagnostics area instead: ",
...
"tab.voice": "Voice",
      ...
      function VoiceSettingsPanel() {
        ...
        const [diag, setDiag] = react.useState(null); // { running, checks, error, recent, worker }
        const runDiagnose = async () => {
          setDiag({ running: true, checks: null, error: null, recent: [], worker: null });
          try {
            ...
            setDiag({ running: false, checks: d.checks || [], error: null, recent: d.recent || [], worker: d.worker || null });
            return d;
          } catch (e) {
            setDiag({ running: false, checks: null, error: String((e && e.message) || e), recent: [], worker: null });
            return null;
          }
        };
        // ---- 一键复制诊断日志 ----
        // Labels are English-only on purpose: the i18n test forbids CJK
        // literals outside the dictionary, and the log is developer-facing.
        // Values are runtime data (may contain Chinese) — no message text or
        // API keys are ever included, only lengths and flags.
        const buildDiagLog = payload => {
          const L = [];
          L.push("# dsh-tts diagnostic log");
          L.push("time: " + new Date().toISOString());
          try { L.push("ua: " + navigator.userAgent); } catch (e) {}
          L.push("settings: provider=" + shared.provider + " voice=" + shared.voice +
            " rvc.baseUrl=" + shared.rvc.baseUrl +
            " index.baseUrl=" + shared.index.baseUrl +
            " cosy.baseUrl=" + shared.cosy.baseUrl +
            " cloud.voice=" + (shared.cloud.voice || "") +
            " cloud.apiKeySet=" + (!!shared.cloudHost.apiKeySet));
          L.push("checks:");
          for (const c of (payload && payload.checks) || []) {
            L.push("  [" + (c.ok ? "ok" : c.cls || "fail") + "] " + c.name + " -- " + (c.detail || ""));
          }
          L.push("worker: " + JSON.stringify((payload && payload.worker) || null));
          L.push("host.recent (" + (((payload && payload.recent) || []).length) + "):");
          for (const e of (payload && payload.recent) || []) {
            L.push("  " + new Date(e.at).toISOString() +
              " " + e.provider + " " + e.voice +
              " len=" + e.textLen + " " + (e.stage || "") +
              (e.ms !== undefined ? " " + e.ms + "ms" : "") +
              " :: " + (e.error || ""));
          }
          L.push("client.recent (" + shared.diagLog.length + "):");
          for (const e of shared.diagLog) {
            L.push("  " + new Date(e.at).toISOString() +
              " " + (e.kind || "") +
              (e.provider ? " " + e.provider : "") +
              (e.voice ? " " + e.voice : "") +
              (e.textLen !== undefined ? " len=" + e.textLen : "") +
              (e.ms !== undefined ? " " + e.ms + "ms" : "") +
        ...
        const copyDiagLog = async () => {
          try {
            const payload = diag && diag.checks
              ? { checks: diag.checks, recent: diag.recent || [], worker: diag.worker || null }
              : await runDiagnose();
            if (!payload || !payload.checks) throw new Error(t("diag.fail") + ((diag && diag.error) || "?"));
            const text = buildDiagLog(payload);
            let copied = false;
            try {
              ...
              try { document.body.removeChild(ta); } catch (e3) {}
            }
            if (!copied) throw new Error(t("diag.copyFail"));
            showToast(t("diag.copied"), "ok");
          } catch (e) {
