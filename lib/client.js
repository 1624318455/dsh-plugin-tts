...
window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const CLIENT_BUILD = "2026-09-13.1-lively-age";
    const apply = ctx => {
      ...
      const I18N = {
        ...
        dict: {
          zh: {
            ...
            "rvcEntry.title": "需要克隆音色？",
            "rvcEntry.desc": "基础朗读开箱即用；只有需要用自己的模型朗读时才展开自定义音色（RVC）",
            "rvcEntry.open": "展开自定义音色（RVC）",
            "field.voice": "朗读音色",
...
"tab.voice": "语音",
          ...
          en: {
            ...
            "rvcEntry.title": "Need a cloned voice?",
            "rvcEntry.desc": "Basic reading works out of the box; expand custom voices (RVC) only when you need your own model",
            "rvcEntry.open": "Show custom voice (RVC)",
...
"tab.voice": "Voice",
      ...
      function saveSettings() {
        try {
          ...
          // UI prefs + the rest of shared.rvc stay local.
      ...
      // ---------- sentence splitting (for scroll-center tracking) ----------
      // Split a cleaned plain-text string into sentence-level segments so the
      // scroll controller can map each TTS chunk back to a DOM text range.
      ...
      async function speakText(rawText, source, onError, opts) {
        ...
        shared.readingSentences = splitSentences(trimmed);
        shared.readingSentenceIdx = 0;
        notify();
      ...
      async function downloadText(rawText, onError) {
        ...
        if (shared.provider === "rvc" && plain.length > 60) {
      ...
      const CSS =
        ...
        ".dsh-tts-reading-bar{display:inline-flex;align-items:center;gap:7px;padding:6px 14px;border-radius:10px;background:var(--dsw-alias-bg-layer-3);border:1px solid var(--dsw-alias-border-l2);box-shadow:0 3px 16px rgba(0,0,0,.22),0 0 30px rgba(0,0,0,.08);font-size:12px;line-height:1;color:var(--dsw-alias-label-primary);white-space:nowrap;transition:transform .35s cubic-bezier(.4,0,.2,1),opacity .35s ease;transform:translateX(12px);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px)}" +
        ".dsh-tts-reading-overlay[data-visible] .dsh-tts-reading-bar{transform:translateX(0)}" +
        ...
        /* ---- sentence highlight (text-shadow projection) ---- */
        ".dsh-tts-sentence-active{transition:text-shadow .6s cubic-bezier(.4,0,.2,1),color .6s cubic-bezier(.4,0,.2,1);text-shadow:0 0 8px rgba(var(--dsw-alias-brand-primary-rgb,79,140,255),.55),0 2px 12px rgba(0,0,0,.18);color:var(--dsw-alias-label-primary)}" +
        ".dsh-tts-sentence-prev{transition:text-shadow .5s cubic-bezier(.4,0,.2,1),color .5s cubic-bezier(.4,0,.2,1);text-shadow:none}";
      ...
      function TtsReadingOverlay() {
        useI18n();
        useSharedForce();
        const visible = shared.speaking && shared.readingIndex != null && shared.chunkProgress && shared.chunkProgress.total > 1;
        const idx = shared.readingIndex || 0;
        const total = (shared.chunkProgress && shared.chunkProgress.total) || 0;

        // ---------- scroll-to-center: best-effort scroll the conversation ----------
        // Find the conversation container and try to scroll the current message
        // element so it sits near the vertical center of the viewport.
        react.useEffect(() => {
          if (!visible || idx <= 0) return;
          try {
            // 1. Locate the scrollable conversation container.
            //    DSH renders conversation messages inside a scrollable div;
            //    try common patterns.
            const containers = document.querySelectorAll(
              '[class*="conversation"],[class*="messages"],[class*="chat-scroll"],[class*="_scroll"]'
            );
            let scrollEl = null;
            ...
            if (!scrollEl) return;
            // 2. Find the currently visible assistant message elements.
            const msgs = scrollEl.querySelectorAll('[class*="message"],[class*="msg"],[data-message-id]');
            if (!msgs.length) return;
            // 3. Estimate which message corresponds to the current reading position
            //    by proportion: readingIndex / total maps to position in the
            //    message list.
            const ratio = total > 0 ? idx / total : 0;
            const targetIdx = Math.min(Math.floor(ratio * msgs.length), msgs.length - 1);
            const target = msgs[targetIdx];
            if (!target) return;
            // 4. Smooth-scroll so the target is roughly centered.
            const containerRect = scrollEl.getBoundingClientRect();
            const targetRect = target.getBoundingClientRect();
            const offsetInContainer = targetRect.top - containerRect.top + scrollEl.scrollTop;
            const scrollTo = offsetInContainer - containerRect.height / 2 + targetRect.height / 2;
            scrollEl.scrollTo({ top: Math.max(0, scrollTo), behavior: "smooth" });
          } catch (e) { /* best-effort, never break the overlay */ }
        }, [visible, idx, total]);

        // Extract a short preview of the text being read (first ~40 chars from
        // the currentText). This is best-effort: plainText already stripped
        // markdown, so we just take a prefix.
        let preview = "";
        if (visible && shared.currentText) {
          preview = shared.currentText.slice(0, 50);
          if (shared.currentText.length > 50) preview += "…";
        }
        ...
        return react.createElement(
          ...
          react.createElement(
            ...
            react.createElement(
              ...
              t("overlay.reading") + "  " + idx + " / " + total,
            ),
          ),
          preview
            ? react.createElement(
                "div",
                { className: "dsh-tts-reading-preview" },
                preview,
              )
            : null,
        );
      ...
      // ---------- scroll-to-center controller (headless, no UI) ----------
      // Actively controls the conversation scroll bar so the sentence
      // currently being read stays vertically centred in the viewport.
      // Also re-centres on layout changes (resize / reflow / image load).

      // Locate the scrollable conversation container.
      function _ttsFindScrollContainer() {
        try {
          // DSH host renders conversation inside a scrollable div.  Try
          // several common class-name patterns used across DSH versions.
          const sels = [
            '[class*="conversation"]',
            '[class*="messages"]',
            '[class*="chat-scroll"]',
      ...
      // Walk the DOM inside `root` and return the deepest element whose
      // textContent contains `needle`.  We prefer "deepest" because a
      // <p> inside a <div> is more precise than the <div> itself.
      function _ttsFindTextNode(root, needle) {
        if (!root || !needle) return null;
        const preview = needle.slice(0, 120);
        let best = null;
        let bestDepth = -1;
          ...
          NodeFilter.SHOW_ELEMENT,
          {
            acceptNode(node) {
              try {
                ...
                if (t && t.includes(preview)) return NodeFilter.FILTER_ACCEPT;
              } catch (e) {}
        ...
        while ((node = walk.nextNode())) {
          // Track depth by counting parents
          let d = 0;
          let p = node;
          ...
          // Stop after a reasonable search
          if (d > 15) break;
        }
        return best;
      }

      // Map a (chunkIndex, chunkTotal) pair to a sentence index inside
      // the shared.readingSentences array.  The server splits text into
      // chunks roughly by paragraph; sentences within a chunk are evenly
      // distributed across the chunk's time span, so a proportional
      // mapping works well enough.
      function _ttsChunkToSentenceIdx(chunkIdx, chunkTotal, sentenceCount) {
        if (!sentenceCount || !chunkTotal) return 0;
        const ratio = (chunkIdx - 1) / Math.max(chunkTotal - 1, 1);
        return Math.min(Math.round(ratio * (sentenceCount - 1)), sentenceCount - 1);
      }

      // Core: scroll the conversation so the current reading sentence
      // sits at the vertical centre of the viewport.
      let _ttsLastScrollRAF = 0;
      let _ttsHighlightedEl = null; // currently highlighted sentence DOM node
      function _ttsScrollToCenter() {
        if (!shared.speaking || shared.readingIndex == null) return;
        const cp = shared.chunkProgress;
        const sentences = shared.readingSentences;
        if (!cp || !sentences || !sentences.length) return;

        const scrollEl = _ttsFindScrollContainer();
        if (!scrollEl) return;

        // Map chunk index → sentence index
        const sIdx = _ttsChunkToSentenceIdx(
          shared.readingIndex, cp.total, sentences.length,
        );
        shared.readingSentenceIdx = sIdx;
        const needle = sentences[sIdx];
        if (!needle) return;

        // Find the DOM node for this sentence
        const target = _ttsFindTextNode(scrollEl, needle);
        if (!target) return;

        // --- Sentence highlight (text-shadow projection) ---
        if (_ttsHighlightedEl !== target) {
          // Fade out previous highlight
          if (_ttsHighlightedEl) {
            _ttsHighlightedEl.classList.remove("dsh-tts-sentence-active");
            _ttsHighlightedEl.classList.add("dsh-tts-sentence-prev");
            // Remove prev class after transition completes so it doesn't
            // linger if the user later scrolls back to this element.
            const prev = _ttsHighlightedEl;
            setTimeout(() => {
              if (prev.classList) prev.classList.remove("dsh-tts-sentence-prev");
            }, 550);
          }
          // Fade in new highlight
          target.classList.remove("dsh-tts-sentence-prev");
          target.classList.add("dsh-tts-sentence-active");
          _ttsHighlightedEl = target;
        }

        // Compute the scroll offset that centres `target` vertically.
        const containerRect = scrollEl.getBoundingClientRect();
        const targetRect = target.getBoundingClientRect();
        // targetRect is in viewport coords; convert to scroll-container coords
        const targetTop = targetRect.top - containerRect.top + scrollEl.scrollTop;
        const targetMid = targetTop + targetRect.height / 2;
        const targetScrollTo = targetMid - containerRect.height / 2;

        // Cancel any pending animation frame to avoid stacking
        if (_ttsLastScrollRAF) cancelAnimationFrame(_ttsLastScrollRAF);
        _ttsLastScrollRAF = requestAnimationFrame(() => {
          _ttsLastScrollRAF = 0;
          scrollEl.scrollTo({
            top: Math.max(0, targetScrollTo),
            behavior: "smooth",
          });
        });
      }

      // Clean up highlight when speaking stops.
      ...
      function TtsScrollController() {
        useSharedForce();

        // --- Re-centre whenever the reading chunk changes ---
        react.useEffect(() => {
          if (!shared.speaking || shared.readingIndex == null) {
            _ttsClearHighlight();
            return;
          }
          _ttsScrollToCenter();
        }, [shared.readingIndex, shared.chunkProgress && shared.chunkProgress.index]);

        // --- Re-centre on layout changes (resize / reflow / image load) ---
        react.useEffect(() => {
          if (!shared.speaking || shared.readingIndex == null) return;
          const scrollEl = _ttsFindScrollContainer();
          if (!scrollEl) return;
          ...
          const recenter = () => {
            ...
            resizeTimer = setTimeout(() => _ttsScrollToCenter(), 80);
          };
        ...
        }, [shared.speaking, shared.readingIndex]);

        // --- Clean up highlight when speaking fully stops ---
        react.useEffect(() => {
          if (!shared.speaking) {
            // Delay cleanup so the fade-out transition has time to play
      ...
      slots.inject("shell.overlay", () =>
        slots.register(
          { name: "shell.overlay", key: "tts-scroll-ctrl", id: "tts-scroll-ctrl", order: 1004 },
          TtsScrollController,
        ),
      );
