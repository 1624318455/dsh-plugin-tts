...
"tab.voice": "Voice",
      ...
      // Core: scroll the conversation so the current reading position
      ...
      function _ttsScrollToCenter() {
        ...
        if (!cp || !cp.total) return;

        const scrollEl = _ttsFindScrollContainer();
        if (!scrollEl) return;

        // --- Find the message block being read ---
        // Search for any element whose textContent contains the beginning of
        // the current text.  We match broadly because DOM structure varies
        // across DSH versions.
        const needle = (shared.currentText || "").slice(0, 100);
        if (!needle) return;

        let msgEl = null;
        // Try common message selectors first
        const sels = [
          "[class*='message']",
          "[class*='msg']",
          "[data-message-id]",
        ...
        for (const sel of sels) {
          ...
          for (const el of els) {
            ...
            if (t && t.length > 30 && t.includes(needle.slice(0, 40))) {
              msgEl = el;
              break;
            }
          }
        ...
        // Fallback: any element containing the needle
        if (!msgEl) {
          ...
          while ((n = walker.nextNode())) {
            try {
              ...
              if (t && t.includes(needle.slice(0, 40))) { msgEl = n; break; }
            } catch (e) {}
          }
        }
        if (!msgEl) return;

        // --- Apply shadow highlight to the message block ---
        if (_ttsHighlightedEl !== msgEl) {
          ...
          msgEl.classList.remove("dsh-tts-sentence-prev");
          msgEl.classList.add("dsh-tts-sentence-active");
          _ttsHighlightedEl = msgEl;
        }

        // --- Proportional scroll: position within the message block ---
        const containerRect = scrollEl.getBoundingClientRect();
        const msgRect = msgEl.getBoundingClientRect();
        // msgRect.top is in viewport coords; convert to scroll-container coords
        const msgTop = msgRect.top - containerRect.top + scrollEl.scrollTop;
        const msgHeight = msgRect.height;
        // readingIndex is 1-based, total is the number of chunks
        const ratio = (shared.readingIndex - 1) / Math.max(cp.total - 1, 1);
        // Target position: proportional offset within the message block
