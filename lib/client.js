...
window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const CLIENT_BUILD = "2026-09-20.2-follow-paused-nohighlight";
    const apply = ctx => {
...
"tab.voice": "Voice",
      ...
      function TtsReadingOverlay() {
        ...
        // --- Scroll-follow: keep the reading message visible ---
        // Old logic only scrolled on chunk change; within a long chunk the
        // effect never re-ran, so reading drifted out of view. Now: scroll on
        // chunk change AND poll while speaking — if the target has drifted out
        // of (or near the edge of) the visible area, nudge it back to centre.
        // No shadow highlight: the box-shadow projection looked bad, removed.
        const keepVisible = () => {
          if (!shared.showScrollFollow || !shared.speaking) return false;
          const rIdx = shared.readingIndex;
          const progress = shared.chunkProgress;
          if (rIdx == null || !progress || !progress.total) return false;
          try {
        ...
        return react.createElement(
          ...
          react.createElement(
            ...
            react.createElement(
              ...
              (shared.paused ? t("overlay.paused") : t("overlay.reading")) + "  " + idx + " / " + total,
            ),
          ),
        );
      }
