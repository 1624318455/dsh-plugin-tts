...
"tab.voice": "Voice",
      ...
      function TtsReadingOverlay() {
        ...
        const idx = readingIdx || 0;
        const total = (cp && cp.total) || 0;

        // --- Scroll-to-centre + shadow highlight ---
        // Runs whenever readingIndex changes, regardless of overlay visibility
        react.useEffect(() => {
          if (!shared.showScrollFollow || !speaking || readingIdx == null || !cp || !cp.total) return;
          try {
            ...
            const ratio = (readingIdx - 1) / Math.max(cp.total - 1, 1);
