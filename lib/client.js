...
window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const CLIENT_BUILD = "2026-09-26.03-early-done-log";
    const apply = ctx => {
...
"tab.voice": "Voice",
      ...
      const shared = {
        ...
        readingCenterOnce: false, // first follow tick centres the message (click feedback)
      ...
      async function speakText(rawText, source, onError, opts) {
        ...
        shared.chunkStartedAt = Date.now();
        // Centre the message on the first follow tick (instant feedback).
        shared.readingCenterOnce = true;
      ...
      function TtsReadingOverlay() {
        ...
        const keepVisible = () => {
          ...
          try {
            ...
            // First tick after (re)anchor: centre the message unconditionally
            // so clicking read gives instant visible feedback. Afterwards the
            // deadband takes over for smooth tracking.
            if (shared.readingCenterOnce) {
              shared.readingCenterOnce = false;
              const scrollTo = msgTop + msgHeight * Math.min(0.5, Math.max(0, ratio)) - containerRect.height / 2;
