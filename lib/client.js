      ...
      const I18N = {
        ...
        dict: {
          zh: {
            ...
            "host.filesNeedsServer": "浏览文件需要先启动本地 RVC 服务。\n\n请先运行 rvc-server.py 并保持终端窗口开启：\n{startup}\n\n若服务已启动，请确认「服务地址」与启动端口一致（当前：{baseUrl}）。\n原始错误：{error}",
...
"tab.voice": "语音",
          ...
          en: {
            ...
            "host.filesNeedsServer": "Browsing files requires the local RVC service to be running first.\n\nStart rvc-server.py and keep the terminal window open:\n{startup}\n\nIf it is already running, check that the Service URL matches the port (currently {baseUrl}).\nRaw error: {error}",
            "host.chunkFail": "Later-section synthesis failed",
            "host.compactNeedsServer": "Compacting the index requires the local RVC service to be running first.\n\nStart rvc-server.py and keep the terminal window open:\n{startup}\n\nIf it is already running, check that the Service URL matches the port (currently {baseUrl}).\nRaw error: {error}",
