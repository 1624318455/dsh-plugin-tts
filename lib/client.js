...
"tab.voice": "Voice",
      ...
      // Map a chunk index to a paragraph index.  Since both server chunks
      // and client paragraphs split on paragraph boundaries, we use a direct
      // 1:1 mapping clamped to array bounds.
      function _ttsChunkToParagraphIdx(chunkIdx, paragraphCount) {
        if (!paragraphCount) return 0;
        return Math.min(chunkIdx - 1, paragraphCount - 1);
      }
      ...
      let _ttsHighlightedEl = null; // currently highlighted DOM node
      function _ttsScrollToCenter() {
        if (!shared.speaking || shared.readingIndex == null) return;
        const cp = shared.chunkProgress;
        const paragraphs = shared.readingSentences;
        if (!cp || !paragraphs || !paragraphs.length) return;

        const scrollEl = _ttsFindScrollContainer();
        if (!scrollEl) return;

        // Direct 1:1 mapping: readingIndex (1-based) → paragraph array index
        const pIdx = _ttsChunkToParagraphIdx(shared.readingIndex, paragraphs.length);
        shared.readingSentenceIdx = pIdx;
        const needle = paragraphs[pIdx];
        if (!needle) return;

        // Find the DOM node for this paragraph.
        // Strategy 1: look for a <p> or similar block element whose text
        // starts with the same first 80 chars as our paragraph.
        const preview = needle.slice(0, 80);
        let target = null;
        const candidates = scrollEl.querySelectorAll(
          "p, li, td, th, pre, blockquote, [class*='message'], [class*='paragraph'], [class*='text'], [class*='content']"
        );
        for (const el of candidates) {
          const t = (el.innerText || el.textContent || "").trim();
          if (t && t.startsWith(preview)) { target = el; break; }
        }
        // Fallback: TreeWalker for any element whose text starts with preview
        if (!target) {
          ...
          while ((n = walker.nextNode())) {
            try {
              ...
              if (t && t.startsWith(preview)) { target = n; break; }
            } catch (e) {}
          }
        }
        // Last fallback: old includes-based search with deepest match
        if (!target) {
          const walker = document.createTreeWalker(scrollEl, NodeFilter.SHOW_ELEMENT);
          let best = null, bestDepth = -1, n;
          while ((n = walker.nextNode())) {
            try {
              const t = (n.innerText || n.textContent || "").trim();
              if (t && t.includes(preview.slice(0, 40))) {
                let d = 0, p = n;
                while (p && p !== scrollEl) { d++; p = p.parentElement; }
      ...
      function TtsScrollController() {
        ...
        react.useEffect(() => {
          ...
          if (shared.showScrollFollow) _ttsScrollToCenter();
        }, [shared.readingIndex, shared.chunkProgress && shared.chunkProgress.index, shared.showScrollFollow]);
