    ...
    const apply = ctx => {
      ...
      function VoiceSettingsPanel() {
        ...
        const fetchPacks = async () => {
          ...
          try {
            const r = await fetch(
              "/dsh-tts-api/rvc-packs?registry=" + encodeURIComponent(reg),
            );
            const d = await r.json().catch(() => null);
            if (!r.ok || !d || d.error) {
        ...
        const installPack = async pack => {
          ...
          setPackNote(null);
          try {
            const r = await fetch("/dsh-tts-api/rvc-pack-install", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ registry: registryUrl.trim(), packId: pack.id }),
            });
            const d = await r.json().catch(() => null);
            if (!r.ok || !d || d.error) {
              throw new Error((d && d.error) || "HTTP " + r.status);
            ...
            setPackNote({
              ok: false,
              text: "安装失败：" + String((e && e.message) || e),
            });
          }
          setInstalling(null);
        };
          ...
          react.createElement(
            ...
            react.createElement(
              ...
              react.createElement(
                ...
                "从音色包仓库一键下载安装音色（模型 + 紧凑索引），安装后自动启用",
              ),
            ),
            field(
              "仓库地址",
              ...
              "仓库目录需包含 manifest.json（音色包清单）；下载由插件代劳并做 sha256 校验",
            ),
            packs && packs.loading
              ? react.createElement(
                  "div",
            ...
            react.createElement(
              ...
              "仅安装版权干净（可分发）的音色；演示音色 azusa-test 受版权限制不出现在公开仓库。",
            ),
          );
        // ---- 紧凑索引生成器 ----
