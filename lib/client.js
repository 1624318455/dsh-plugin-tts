...
"tab.voice": "Voice",
      ...
      function pickNodes(snap) {
        ...
        if (Array.isArray(snap.nodes)) return snap.nodes;
        if (Array.isArray(snap.messages)) return snap.messages;
        if (Array.isArray(snap.items)) return snap.items;
        if (snap.session && typeof snap.session === "object") {
          if (Array.isArray(snap.session.nodes)) return snap.session.nodes;
          if (Array.isArray(snap.session.messages)) return snap.session.messages;
        }
        if (snap.data && typeof snap.data === "object") {
          if (Array.isArray(snap.data.nodes)) return snap.data.nodes;
          if (Array.isArray(snap.data.messages)) return snap.data.messages;
        }
        return null;
      }
      ...
      function nodeIdOf(n) {
        ...
        for (const k of keys) {
          if (typeof n[k] === "string" && n[k]) return n[k];
        }
        return null;
      }
      ...
      // prop/snapshot shape. Returns "" when nothing matches (the caller then
      // tries the DOM fallback before giving up).
      function resolveMessageText(props, snap) {
        props = props || {};
        ...
        const nodes = pickNodes(snap) || pickNodes(props.session);
      ...
      // next to our own action button (our buttons are stripped before read).
      function domTextForEl(el) {
        try {
          if (!el || !el.closest) return "";
          const root = el.closest(
            "[data-message-id],[data-node-id],article,[data-role]",
          ) || el.parentElement;
          if (!root) return "";
          const clone = root.cloneNode(true);
          let btns = [];
          try {
            btns = clone.querySelectorAll
              ? clone.querySelectorAll("button,.dsh-tts-mini,.dsh-tts-action")
              : [];
          } catch (e) { btns = []; }
          for (const b of btns) {
      ...
      // One-line mount log per slot so a future contract skew can be diagnosed
      // from the web console (which prop keys the host actually passed).
      const loggedSlots = {};
      function logSlotPropsOnce(slot, props) {
        if (loggedSlots[slot]) return;
        loggedSlots[slot] = true;
        try {
          const keys = props ? Object.keys(props) : [];
          console.info("[tts] slot mounted: " + slot + " props: " + keys.join(","));
        } catch (e) {}
      }
      ...
      function AutoReadToggle(props) {
        useI18n();
        logSlotPropsOnce("conversation.input.left", props);
        ...
        // Session snapshot from EITHER the legacy props.session object OR the
        // standard useSession hook (single call; identity selector never
        // throws on an empty snapshot, unlike s => s.nodes).
        let snap = null;
        if (props && typeof props.useSession === "function") {
          try {
            snap = props.useSession(s => s);
          } catch (e) { snap = null; }
        }
        const session = (props && props.session) ||
          (snap && snap.session) || snap || null;
        const key = (props && props.sessionId) ||
          (snap && snap.sessionId) || "default";
        react.useEffect(() => {
          const nodes = pickNodes(session) || pickNodes(snap);
          if (!nodes) return;
          let maxSeq = -1;
          let newest = null;
          let idx = 0;
      ...
      function ReadAloudButton(props) {
        useI18n();
        useSharedForce();
        logSlotPropsOnce("conversation.chat.assistant-actions", props);
        ...
        const raw = resolveMessageText(props, snap);
