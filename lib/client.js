...
"tab.voice": "Voice",
      ...
      function pickNodes(snap) {
        ...
        if (snap.conversation && typeof snap.conversation === "object") {
          if (Array.isArray(snap.conversation.nodes)) return snap.conversation.nodes;
          if (Array.isArray(snap.conversation.messages)) return snap.conversation.messages;
        }
        return null;
      }
      ...
      function describeSnap(snap) {
        try {
          ...
          return "keys:" + keys.slice(0, 12).join("+") +
            ",nodes:" + (nodes ? nodes.length : 0) + head;
        } catch (e) { return "?"; }
      }
      function logSlotPropsOnce(slot, props, snap) {
        try {
          const keys = props ? Object.keys(props).sort().join(",") : "";
          const shape = snap !== undefined ? "|" + describeSnap(snap) : "";
      ...
      function AutoReadToggle(props) {
        ...
        logSlotPropsOnce("conversation.input.left", props, snap);
        react.useEffect(() => {
      ...
      function ReadAloudButton(props) {
        ...
        logSlotPropsOnce("conversation.chat.assistant-actions", props, snap);
