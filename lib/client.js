...
window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const CLIENT_BUILD = "2026-09-20.7-follow-relocate-guard";
    const apply = ctx => {
...
"tab.voice": "Voice",
      ...
      function TtsReadingOverlay() {
        ...
        const viewTopOf = (el) => { try { return el.scrollTop; } catch (e) { return 0; } };
        ...
        const keepVisible = () => {
          ...
          try {
            ...
            // Wrong-element guard: a sane message is taller than a line and
            // its top sits within a screen of the viewport. Otherwise the
            // lookup hit a fragment (e.g. our own overlay text) — skip this
            // tick instead of rubber-banding to the top (scrollTo clamped 0).
            if (msgHeight < 40 || msgTop < viewTopOf(scrollEl) - containerRect.height ||
                msgTop > viewTopOf(scrollEl) + containerRect.height * 2) {
              mark("bad-target");
              return false;
            }
      ...
      function ReadAloudButton(props) {
        ...
        const onClick = () => {
          ...
          // nearest ancestor holding the message text. Follow scrolls THIS
          // element — no more whole-page text guessing (which kept landing
          // on 24px fragments and scrolling to wrong offsets).
          try {
            shared.readingMsgEl = null;
            ...
            while (el && depth < 8) {
              ...
              if (len > 100) best = el; // keep climbing: want the message root, not a line
              if (len > Math.max(500, target.length * 0.6)) break; // big enough = message root
              el = el.parentElement;
              depth++;
            }
            if (best) {
