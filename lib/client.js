...
window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const CLIENT_BUILD = "2026-09-20.14-follow-off-default";
    const apply = ctx => {
...
"tab.voice": "Voice",
      ...
      async function playChunks(jobId, initialUrls, total, token, onError) {
        ...
        const schedule = buf => {
          ...
          src.buffer = buf;
          // Stall guard: if the next chunk is not ready yet, stretch THIS
          // chunk to 0.85x so its tail buys ~15% more time for synthesis.
          // Recovers automatically (one-shot flag consumed below).
          const stall = shared.stallStretch === true;
          if (stall) shared.stallStretch = false;
          src.playbackRate.value = stall ? (shared.rate || 1) * 0.85 : (shared.rate || 1); // mini-player speed
          src.connect(master);
          src.start(nextStart);
        ...
        try {
          ...
          while (token === shared.speakToken && !finished) {
            ...
            requestNext();
            // Watchdog: next chunk still missing while the tail of the current
            // audio is near -> stretch the NEXT scheduled chunk (0.85x, ~15%
            // more time) and show "synthesizing…" instead of going silent.
            // Only arms when something is actually playing (played < decoded:
            // audio in flight) and the next slot is still empty.
            if (!finished && played < decoded && cursor < total && queue[cursor] === undefined && !completed) {
              shared.stallStretch = true;
