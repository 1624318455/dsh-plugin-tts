  ...
  factory: require => {
    ...
    const apply = ctx => {
      ...
      const shared = {
        ...
        audioEl: null,
        spareAudioEl: null, // second <audio> used for ping-pong chunk playback
      ...
      // Progressive chunked playback: plays buffered chunks back-to-back while
      // prefetching the next converted chunk from the host (convert overlaps
      // with playback). Two <audio> elements ping-pong: while chunk N plays on
      // one element, the next chunk's AUDIO DATA is already loading into the
      // other, so the boundary swap is instant (no reload gap per chunk).
