...
"tab.voice": "Voice",
      ...
      // First content list (in AUTO_HOOKS priority) holding an assistant node
      // with real text — the auto-read watch source.
      function pickAutoList(snaps) {
        for (const name of AUTO_HOOKS) {
          for (const entry of snaps) {
            ...
            let hasText = false;
            for (const n of nodes) {
              if (n && typeof n === "object" && isAssistantNode(n) &&
                  extractText(n).trim()) {
                hasText = true;
                break;
              }
      ...
      function describeSnap(snap) {
        try {
          ...
          return "keys:" + keys.slice(0, 12).join("+") +
            " [" + deep + "] nodes:" + (nodes ? nodes.length : 0) + head;
        } catch (e) { return "?"; }
      }
      ...
      function AutoReadToggle(props) {
        ...
        react.useEffect(() => {
          ...
          // pending/partial); (2) user-side growth — a reply causally implies
          // its user message rendered, possibly an earlier render than the
          // reply, so the latch must outlive the growing render. First-sight
          // user nodes only record (they may be history). Idle histories
          // opened after refresh/switch never latch and stay silent at ANY
          // size, including <= SYNC_BULK_NEW tiny ones.
          let lively = shared.livelySeen.get(key) === true;
          if (!lively) {
            for (const entry of snaps) {
              if (snapActive(entry.snap)) {
